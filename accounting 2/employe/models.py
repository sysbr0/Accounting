...
class EmployeePayment(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.pk:  # Check if this is a new record
            ...
            note = f"دفعة كاش ل {self.employee.position} {self.employee.name}، المبلغ {self.amount}. الملاحظة: {self.note}"
            
            # Get or create the default source
            source, created = MoneySource.objects.get_or_create(
                source_type='cash',
        ...
        #        raise ValueError('Insufficient funds in default cash source')
